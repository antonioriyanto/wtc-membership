...
// The first regex might have failed due to whitespace/formatting. Let's do it more robustly.
...
if (!content.includes('navigator.geolocation')) {
  ...
  // 2. Add Haversine Function outside the component (top of file)
  content = content.replace(
    /export default function CustomerMemberView/,
...
export default function CustomerMemberView`
  ...
  // 3. Add useEffect for Geolocation
  content = content.replace(
    /useEffect\(\(\) => \{\n    if \(!db\) return;/,
    `useEffect(() => {
